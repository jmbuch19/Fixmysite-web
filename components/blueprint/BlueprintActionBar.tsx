...
/**
 * Action bar for /plan/blueprint/[id]/full. Two independent surfaces:
 *
 *   1. Download PDF — fetch + blob pattern. Browser-native download
 *      dialog; filename comes from the server's Content-Disposition.
 *   2. Send to developer — email input + submit. Bugbite emails the
 *      blueprint PDF straight to the address, with a peer-to-peer
 *      developer-tone email body.
 *
 * Each action has its own state machine so they run independently —
 * an owner who downloads first can still email a developer without
 * resetting state.
 */
export function BlueprintActionBar({ blueprintId }: { blueprintId: string }) {
  return (
    <section className="rounded-xl border border-zinc-200 bg-white p-6">
      ...
      <p className="mt-2 text-sm leading-relaxed text-zinc-600">
        Save the PDF, or send the blueprint straight to a developer&apos;s
        inbox. Bugbite already emailed your copy when you paid — this is
        for sharing.
      </p>

      <div className="mt-5 space-y-6">
        <DownloadRow blueprintId={blueprintId} />
        <div className="border-t border-zinc-100 pt-6">
          <SendDeveloperRow blueprintId={blueprintId} />
        </div>
      </div>
    </section>
  )
...
function DownloadRow({ blueprintId }: { blueprintId: string }) {
  ...
  async function handleDownload() {
    ...
    try {
      ...
      const filename = match?.[1] ?? `fixmysite-blueprint-${blueprintId}.pdf`
...
function SendDeveloperRow({ blueprintId }: { blueprintId: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/blueprint/send-developer', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (!res.ok) {
        setState({
          phase: 'error',
          message:
            'Could not send. Try again, or email hello@fixmysite.in and Bugbite will forward it for you.',
        })
        return
      }
    ...
    return (
      <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4">
        <p className="text-sm font-semibold text-emerald-900">
          Blueprint sent to {state.email}
        </p>
        <p className="mt-1 text-sm text-emerald-900">
          They&apos;ll receive the PDF in their inbox shortly. Want to send
          it to someone else too?
        </p>
        <button
          type="button"
          onClick={() => {
  ...
  return (
    <div>
      ...
      <form
        ...
        noValidate
      >
        <label htmlFor="blueprint-dev-email" className="sr-only">
          Developer&apos;s email
        </label>
        <input
          id="blueprint-dev-email"
          type="email"
          inputMode="email"
          autoComplete="email"
          required
        ...
        >
          {isSending ? 'Sending…' : 'Send blueprint'}
        </button>
      </form>
      {state.phase === 'error' && (
        <p className="mt-2 text-sm text-red-700" role="alert">
